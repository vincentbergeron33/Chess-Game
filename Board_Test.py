...
class Boardtest(unittest.TestCase):
    ...
    def test_pawn_can_capture(self):
        ...
        for rows in board.pieces:
            for piece in rows:
                if piece is not None:
                    dictionary_moves[piece.location] = piece.movement(board_test)
        all_moves = []
        for rows in board.pieces:
            for piece in rows:
                if piece is not None:
                    all_moves = all_moves + piece.movement(board_test)
        

        #piece_to_kill_location = self.corresponding_keys(white_king_location, dictionary_moves)
        #print(piece_to_kill_location)
        Piece.checkmate_capture(self, board_test)
